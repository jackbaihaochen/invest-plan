...
CATS = ["指数基金", "杠杆ETF", "美股个股", "日本个股", "黄金",
        "债券", "加密货币", "现金·MMF", "持株会", "银行存款"]
ACCTS = ["NISAつみたて投資枠", "NISA成長投資枠", "特定口座", "一般口座", "其他"]

# 从用户导出的楽天証券取引履歴里还原出来的实际持仓
HOLDINGS = [
    ("楽天・プラス・S&P500",            "指数基金", "NISAつみたて投資枠", "日元"),
    ("楽天・S&P500",                    "指数基金", "NISA成長投資枠",     "日元"),
    ("eMAXIS Slim 全世界株式(オルカン)", "指数基金", "NISAつみたて投資枠", "日元"),
    ("eMAXIS Slim 米国株式(S&P500)",     "指数基金", "NISA成長投資枠",     "日元"),
    ("iFreeNEXT FANG+",                 "指数基金", "NISA成長投資枠",     "日元"),
    ("楽天・プラス・NASDAQ-100",         "指数基金", "NISA成長投資枠",     "日元"),
    ("HSBCインド・インフラ株式",         "指数基金", "特定口座",           "日元"),
    ("Direxion S&P500 3倍 (SPXL)",      "杠杆ETF",  "特定口座",           "美元"),
    ("MICROSOFT",                       "美股个股", "NISA成長投資枠",     "美元"),
    ("ALPHABET C",                      "美股个股", "NISA成長投資枠",     "美元"),
    ("NVIDIA",                          "美股个股", "NISA成長投資枠",     "美元"),
    ("APPLE",                           "美股个股", "NISA成長投資枠",     "美元"),
    ("AMAZON",                          "美股个股", "NISA成長投資枠",     "美元"),
    ("AMD",                             "美股个股", "特定口座",           "美元"),
    ("TAIWAN SEMI ADR",                 "美股个股", "NISA成長投資枠",     "美元"),
    ("VERTIV HOLDINGS",                 "美股个股", "NISA成長投資枠",     "美元"),
    ("CELESTICA",                       "美股个股", "NISA成長投資枠",     "美元"),
    ("CIRCLE (サークル)",               "美股个股", "特定口座",           "美元"),
    ("INVESCO QQQ",                     "美股个股", "NISA成長投資枠",     "美元"),
    ("メイコー",                        "日本个股", "NISA成長投資枠",     "日元"),
    ("メルカリ",                        "日本个股", "NISA成長投資枠",     "日元"),
    ("ＭＩＲＡＲＴＨ不動産投 (REIT)",    "日本个股", "NISA成長投資枠",     "日元"),
    ("ＧＸゴールド",                    "黄金",     "NISA成長投資枠",     "日元"),
    ("金 定額積立 (現物)",              "黄金",     "特定口座",           "日元"),
    ("米国債 TN 2.75% 2027/04",         "债券",     "特定口座",           "美元"),
    ("楽天・米ドルMMF",                 "现金·MMF", "特定口座",           "美元"),
    ("楽天証券 預り金",                 "现金·MMF", "特定口座",           "日元"),
    ("bitFlyer BTC (0.061912)",         "加密货币", "其他",               "日元"),
    ("野村 持株会",                     "持株会",   "其他",               "日元"),
    ("ゆうちょ銀行",                    "银行存款", "其他",               "日元"),
]
...
for i, (name, val, fmt, note) in enumerate([
    ...
    ("起始累计本金",     8_021_506, YEN, "楽天証券 2024/03〜2025/08 的净入金。野村持株会 / ゆうちょ / BTC 的本金请自行加上。"),
...
for i, (nm, cat, acc, cur) in enumerate(HOLDINGS):
    r = 3 + i
...
put(mon, "I5", "「总资产」需要你填 —— 楽天証券首页那个总额 + 野村 + BTC + ゆうちょ。",
...
label(d, 41, "已经用掉")
put(d, "C41", f'=SUMIF({H_ACC},"NISA*",{H_VAL})', font=val, fmt=YEN)
...
put(d, "D42", '="按买入额算约 832万已用"', font=Font(size=9, italic=True, color=MUTED))
...
for i, t in enumerate([
    "7% 是美股长期平均。换成日元还有汇率，跌的年份也很正常。",
    "特定口座的收益要交 20.315% 的税。NISA 生涯额度 1800万，优先填满它更划算。",
    "SPXL 是 3 倍杠杆 ETF，长期持有有波动损耗；你还有信用保証金的进出。这两样放一起风险不小。",
    "野村是持株会 —— 工资和股票来自同一家公司。别让它太大。",
