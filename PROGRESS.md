...
**Current Status**：`claude/rebuild` 分支。**第 1 阶段（本地可用的看板）完成**，51 项测试。
...
（保有商品 CSV を取り込むたびに1点、今は 2026-08-29 の 1 点だけ）、
これからの2本（実績ペース 17万/月 → 2042年9月、計画 40万/月 → 2037年2月）。
過去の評価額は復元できないので描かない —— 見 `docs/decisions.md`。

新しい数字：実績年率 **22.6%**（金額加重、2024/03/25〜2026/08/29）、
本金 903万 / 収益 402万（総資産の 30.8%）、
逆算「2036年 42.6万/月・2041年 19.8万/月・2046年 9.1万/月」。
年率は設定（0/3/5/7/10%）に出した —— 実績 22.6% と既定 7% の差を黙って抱えない。
...
在浏览器里用真实 CSV 实机确认：総資産 ***、累计投入 ***、
按近 6 个月平均 17.1 万/月 → 2042 年 9 月、落后计划 5 年 7 个月、
记入 5 万显示「提前 6.2 天」（与钉住的测试一致）。

**接缝生效**：记入 5 万后环显示 33%，并标注「其中 5 万是你自己记的，等 CSV 确认」。
...
- **見通しと環に同じ数（中央値）を使っていた** —— 问的不是同一件事。见通し用近 6 个月
  **平均** ***（实际积累速度），环用**中央值** 152,794（大多数月份打得赢的目标）。

验证用的 dev 夹具已删除。要再复现：`cp tests/fixtures/rakuten/*.csv public/_dev/` 后
在页面控制台 fetch 进 localStorage，或者直接把 `tests/fixtures/rakuten/` 里的文件拖进页面。
